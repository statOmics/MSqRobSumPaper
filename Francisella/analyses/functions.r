...
read_mq = function(txt_path){
  ...
  fd = fData(set) %>%
    transmute(protein =  GI.number,
              sequence = Sequence,
              reverse = ifelse(is.na(Reverse),'',Reverse) == '+',
              contaminant = ifelse(is.na(Contaminant),'',Contaminant) == '+')
  ...
  pd = read_tsv(path_annot)
  pd = dplyr::rename(pd,condition = genotype)
  pd = column_to_rownames(pd, 'run')
...
####################
  ...
  fData(set) = fData(set)[,! colnames(fData(set)) %in%
                           c('contaminant','reverse', 'site_only','sequence'),drop = FALSE]
  set
}
...
perseus2res = function(perseus_file,fasta_path){
  perseus_file %>%
    ...
    mutate(statistic = str_replace(statistic, "Welch's T-test ",'')
         , statistic = str_replace(statistic, "Test statistic",'t')) %>%
    filter(str_detect(statistic, ' t | q-value | p-value | Difference')) %>%
    mutate(statistic = str_replace(statistic,'N: ','')) %>%
...
############################
DEP_preprocess = function(txt_path){
...
##################################
  ...
  proteinGroups<- read_tsv(str_glue("{txt_path}proteinGroups.txt")) %>%
    as_tibble(.name_repair = 'universal') %>%
    mutate(Gene.names = Protein.IDs)
  ## annot = data_frame(condition = rep(letters[1:5], each = 4)
  ##                  , replicate = rep(1:4,5)
  ##                  , label = paste0(condition,replicate))
## glimpse(infile)
  annot = tibble(label = names(infile)[grepl('^Intensity.',names(infile))] %>%
                   str_split('Intensity\\.') %>% sapply(last)
               , condition = sapply(str_split(label,'_'),last)
               , replicate = map_chr(str_split(label,'_'),~{.[[1]]})
                 )

  proteins = DEP::import_MaxQuant(proteinGroups,annot)
  proteins <- DEP::filter_missval(proteins)
  norm <- DEP::normalize_vsn(proteins)

  proteins_MNAR <- DEP::get_df_long(norm) %>% as_data_frame %>%
